...
int main(){
    ...
    */
    ...
    /* Richiesta HTTP da inviare al server 
       ...
       Essa è una Request del tipo Full-Request (RFC 1945). Il server risponderà con una Response del tipo Full-Response (RFC 1945) che contiene la status line, gli header e l'entity body della risposta.
    ...
    */
    ...
    Questo perchè la status-line finisce con un CRLF (carriage return + line feed) e quindi il ciclo legge la status line fino a quando non viene raggiunto il carattere di fine riga '\n' seguito da un possibile '\r'. Il carattere '\r' è opzionale e può essere omesso. Se il carattere '\r' è presente, allora il ciclo si ferma, altrimenti continua a leggere la status line fino a quando non viene raggiunto il carattere di fine riga '\n' seguito da un possibile '\r'.
    ...
    */
    ...
    printf("\nLeggo l'Entity-Body della risposta del server. Il numero di byte dell' Entity-Body che leggo è ENTITY_SIZE = %d\n\n",ENTITY_SIZE);

	for(i=0;i < ENTITY_SIZE && (t = read(s, entity+i, ENTITY_SIZE-i)); i+=t); // Legge l'entity body della risposta del server

	entity[i]=0; // Termina la stringa con il carattere di fine stringa '\0'. 
	printf("%s",entity); // Stampa l'entity body della risposta del server


    close(s); // Chiude il socket "s"
